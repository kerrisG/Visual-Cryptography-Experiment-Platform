...
unsigned char* AES::Cipher(unsigned char* input, unsigned char *output)
{
	...
	for(i=1; i<=10; i++)         //10轮
	{
		SubBytes(state);         //字节代换
		ShiftRows(state);        //行位移
		if(i!=10)MixColumns(state);//非第10轮，列混淆
...
unsigned char* AES::InvCipher(unsigned char* input, unsigned char *output)
{
	...
	for(r=0; r<4; r++)
	{
		for(c=0; c<4 ;c++)
		{
			state[r][c] = input[c*4+r];  //密文
		}
	}

	AddRoundKey(state, w[10]);     //轮密钥加
	for(i=9; i>=0; i--)            //10轮
	{
		InvShiftRows(state);       //行位移求逆
		InvSubBytes(state);        //字节代换求逆
		AddRoundKey(state, w[i]);  //轮密钥加
		if(i)
		{
			InvMixColumns(state);  //列混淆求逆
		}
	}
...
void AES::KeyExpansion(unsigned char* key, unsigned char w[][4][4])
{
	...
	for(i=1; i<=10; i++)          //10轮
	{
		for(j=0; j<4; j++)        //每轮增加4个新列
		{
			unsigned char t[4];
			for(r=0; r<4; r++)
			{
				t[r] = j ? w[i][r][j-1] : w[i-1][r][3];  //j≠0,t[r]=w[i][r][j-1]; j=0,t[r] = w[i-1][r][3];
			}
			if(j == 0)            //4的倍数
			{
				...
				for(r=0; r<3; r++)
				{
					t[r] = Sbox[t[(r+1)%4]]; //字循环+用S盒进行字节代换
				}
				t[3] = Sbox[temp];
				t[0] ^= rc[i-1];  //轮常量异或
			}
...
void AES::InvSubBytes(unsigned char state[][4])
{
	...
	for(r=0; r<4; r++)
	{
		for(c=0; c<4; c++)
		{
			state[r][c] = InvSbox[state[r][c]]; //逆S盒替换
		}
	}
}
...
AESModeOfOperation::AESModeOfOperation() {  
	m_mode = MODE_ECB;
	m_aes = NULL;
	memset(m_key, 0, 16);  //对定义的字符串进行初始化
	memset(m_iv, 0, 16);
	m_aes = new AES(m_key);
	assert(m_aes != NULL); //如果表达式的值为假，整个程序将退出，并输出一条错误信息。如果表达式的值为真则继续执行后面的语句
}
...
int AESModeOfOperation::Encrypt(unsigned char *_in, int _length, unsigned char *_out) {
	...
	unsigned char *cipherout = new unsigned char[_length];
	int co_index = 0;
	
	if ( _length % 16 == 0) {        // 得到轮数
		rounds = _length / 16;
	} else {
		rounds = _length / 16 + 1;
	}
	
	for (int j = 0; j < rounds; ++j) {       // 每一轮
		start = j*16;
		end = j*16 + 16;
		if (end > _length) end = _length;	// 不再输入
											// 读入明文
...
//			printf("--------------------\n");
			for (int i = 0; i < 16; ++i) {
				if (first_round == true) {
					input[i] = plaintext[i] ^ m_iv[i];  //初始化向量IV和明文分组1进行异或
				} else {
					input[i] = plaintext[i] ^ ciphertext[i]; //明文分组和上一个密文分组异或
				}
			}
			first_round = false;
//			printf("^^^^^^^^^^^^\n");
...
int AESModeOfOperation::Decrypt(unsigned char *_in, int _length, unsigned char *_out) {
	...
	unsigned char output[16] = {0};
	unsigned char plaintext[16] = {0};
	unsigned char *plainout = new unsigned char[_length];
	int po_index = 0 ;
	if (_length % 16 == 0) {        // 得到轮数
		rounds = _length / 16;
	} else {
	...
	for (int j = 0; j < rounds; j++) {       // 每一轮
		...
		memset(ciphertext, 0, 16);        	// 读入密文
		memcpy(ciphertext, _in + start, end - start);
		if ( m_mode == MODE_CFB ) {          //对于不同的模式
			if (first_round = true) {
				m_aes->Cipher(m_iv, output);
				first_round = false;
			} else {
			...
			// TODO
			m_aes->InvCipher(ciphertext, plaintext);
			for (int k = 0; k < end - start; ++k) {
				plainout[po_index++] = plaintext[k];
			}
		}
